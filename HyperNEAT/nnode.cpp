...
NNode::NNode(nodetype ntype,int nodeid) {
	...
	node_id=nodeid;
	ftype=ACTIVATION_FUNCTION_SIGMOID;//(functype)(rand()%ACTIVATION_FUNCTION_END); // Randomly select activation function
	nodetrait=0;
...
NNode::NNode (const char *argline, std::vector<Trait*> &traits) {
	...
	int nodety, nodepl, afunc;
	ss >> node_id >> traitnum >> nodety >> nodepl >> time_const >> bias >> afunc >> x_sub_pos >> y_sub_pos >> x_global >> y_global;
	//std::cout<<" id:"<<node_id<<" trait:"<<traitnum<<" type:"<<nodety<<" place:"<<nodepl<<" time:"<<time_const<<" bias:"<<bias<<" afunc:"<<afunc<<"\n";


	type = (nodetype)nodety;
	gen_node_label = (nodeplace)nodepl;
	ftype = (functype)afunc;
...
NNode::NNode (const NNode& nnode)
{
	...
	// Copy the incoming and outgoing links
	// TODO: Verify that this should remain commented out.
	/*std::vector<Link*>::iterator curlink;

	for(int i = 0; i < nnode.incoming.size(); i++)
		incoming.push_back(new Link(*(nnode.incoming[i])));
	
	for(int i = 0; i < nnode.outgoing.size(); i++)
		outgoing.push_back(new Link(*(nnode.outgoing[i])));*/
}
...
void NNode::flushback_check(std::vector<NNode*> &seenlist) {
	...
	if (!(type==SENSOR)) {
		...
		//std::cout<<"ALERT: "<<this<<" has print  "<<last_activation<<std::endl;
...
void NNode::print_to_file(std::ofstream &outFile) {
	...
	outFile<<type<<" ";
	outFile<<gen_node_label<<" " << time_const << " " << bias << " " << ftype << std::endl;
}
...
void NNode::print_to_file(std::ostream &outFile) {
	...
	char tempbuf2[128];
	sprintf(tempbuf2, "%d %d %f %f %d\n", type, gen_node_label,time_const,bias,ftype);
	outFile << tempbuf2;
}
...
int NNode::depth(int d, Network *mynet) {
  ...
  //Recursion
	...
	// Instantiate an SNode from an input file
	/*SNode::SNode(const char *argline, std::vector<Trait*> &traits) : NNode(argline, traits)
	{
		double ignore;
		std::stringstream ss(argline);

		// Ignore the first 7 values for the node from the input file
		for(int i = 0; i < 7; i++)
			ss >> ignore;

		// After the first 7 values are skipped over, store the x, y and z values of the SNode
		ss >> x_sub_pos >> y_sub_pos >> z_sub_pos;
	}*/
