	...
	if (!iFile) {
		...
		while (!iFile.eof()) 
		{
			...
			if (strcmp(curword,"genomestart")==0) 
			{
				...
				organisms.push_back(new Organism(0,new_genome,gen, metadata));
			...
			else if (strcmp(curword,"/*")==0) 
			{
				...
				int pos=str.find("Generation")+12;
				//std::cout <<pos<<"\t"<<(str.find("Indiv"))<<"\n";
				if (str.find("Indiv")!=std::string::npos)
				{
					gen = atoi(str.substr(pos, str.find("Indiv")-pos).c_str());
				//	std::cout <<"\n"<<gen<<"\n";
				}
...
bool Population::spawn(Genome *g,int size) {
	...
	// (1.0 - (1.0/((double)ACTIVATION_FUNCTION_END))) means chance of changing initial
	// node activation function type is proportional to the number of activation functions
	double afunc_mut_rate = (1.0 - (1.0/((double)ACTIVATION_FUNCTION_END)));
	...
	for(count=1;count<=size-1;count++) {
		...
		new_genome->mutate_node_parameters(0.01,1.0,3.0,1.0,afunc_mut_rate,true);
	...
	// 1/14 risi
	//new_genome=g->duplicate(size); 
	new_organism=new Organism(0.0,g->duplicate(size),1);
	organisms.push_back(new_organism);
