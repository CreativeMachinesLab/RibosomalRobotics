...
double NEAT::afunc_mut_prob=0.1;
    
//ES-HyperNEAT
double NEAT::initialRes = 0;
double NEAT::varianceThreshold= 0;
double NEAT::bandingThreshold= 0;

//double NEAT::divisionThreshold= 0;
double NEAT::maximumRes= 0;

double NEAT::NumMachines= 1;
double NEAT::NumParts = 200;

int NEAT::REQUERY = 0;
int NEAT::HYPERNEAT_SETTING = 0; 
...
bool NEAT::load_neat_params(const char *filename, bool output) {
	...
	paramFile>>curword;
	paramFile>>NEAT::initialRes;

	paramFile>>curword;
	paramFile>>NEAT::varianceThreshold;
	
	paramFile>>curword;
	paramFile>>NEAT::bandingThreshold;
	
	paramFile>>curword;
	paramFile>>NEAT::NumMachines;

	paramFile>>curword;
	paramFile>>NEAT::NumParts;
	
	paramFile>>curword;
	paramFile>>NEAT::maximumRes;
    ...
    if(output) {
	    ...
	    printf("initialRes=%f\n",initialRes);
		printf("varianceThreshold=%f\n",varianceThreshold);
		printf("bandingThreshold=%f\n",bandingThreshold);
		printf("NumMachines=%f\n",NumMachines);
		printf("NumParts=%f\n",NumParts);
		printf("maximumRes=%f\n",maximumRes);
...
// Other activation functions for HyperNEAT (signed)
// Taken from Jason Gauci's implementation of C++ HyperNEAT
double NEAT::fsin(double activesum)
{
	//printf("sin of %f\n", activesum);
	return sin(activesum);
}

double NEAT::fcos(double activesum)
{
	//printf("cos of %f\n", activesum);
	return cos(activesum);
}

double NEAT::fgaussian(double activesum)
{
	//printf("gaussian of %f\n", activesum);
	return exp(-pow(activesum, 2));
}

double NEAT::fsquare(double activesum)
{
	//printf("square of %f\n", activesum);
	return activesum * activesum;
}

double NEAT::fabsroot(double activesum)
{
	//printf("abs root of %f\n", activesum);
	if(activesum < 0.0)
	{
		return -sqrt(fabs(activesum));
	}
	else
	{
		return sqrt(fabs(activesum));
...
double NEAT::flinear(double activesum)
{
	//printf("linear of %f\n", activesum);
	return activesum;
	//return (min(max(activesum, -3.0) ,3.0) / 3.0);
}

/* Removed one's complement - not needed
double NEAT::fonescomplement(double activesum)
{
        double newValue = min(1.0, max(-1.0, activesum / 3.0));
	//printf("one's complement of %f\n", activesum);
	...
	{
		newValue = (-1.0) - activesum;
	}

	return newValue;
}*/
// End other activation functions
