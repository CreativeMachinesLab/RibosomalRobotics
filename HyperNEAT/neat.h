...
namespace NEAT {
	...
	extern double afunc_mut_prob;

	extern double NumMachines;
	extern double NumParts;

	//Evolvable-substrate HyperNEAT additions
	extern double initialRes;
    extern double varianceThreshold;
    extern double bandingThreshold;
    
	extern double maximumRes;

	//0 = FS-HyperNEAT LEO, 1 = ES-HyperNEAT, 2 = Adaptive FS-HyperNEAT, 3 = FS-HyperNEAT with SEF, 4 = SATURATE
	extern int HYPERNEAT_SETTING; 
	...
	extern inline double scale(double val, double min, double max)
	{
		double scaled = val;
	...
	// Other activation functions for HyperNEAT
	extern double fsin(double);
	extern double fcos(double);
	extern double fgaussian(double);
	extern double fsquare(double);
	extern double fabsroot(double);
	extern double flinear(double);
	//extern double fonescomplement(double);
	// End other activation functions

	// max/min functions used in activation functions
	// Returns max of 2 doubles
