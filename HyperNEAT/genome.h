...
namespace NEAT {
	...
	public:
		...
		Network *genesis(int id);

		// Generates the substrate Network from this Genome with specified id and CPPN Network
		Network *genesis(int id, Network *cppn, float LEG_LENGTH);

		//Evolvable-substrate HyperNEAT
		Network *genesisEvolvableSubstrate(int id, Network *cppn);
		...
		void mutate_node_trait(int times);
        	...
        	//jladd
        	void mutate_node_parameters(double tc_power,double tc_rate,double bias_power,double bias_rate,double afunc_rate,bool cold=false);
	...
	/*
		Substrate genome class. Inherits everything from the Genome class, but
		changes the Genome to store SNodes instead of NNodes and modifies the
		Genome::genesis() function to return a Network representing the substrate
		with weights generated by the CPPN.
	*/
	/*class SGenome : public Genome
	{
		public:
			std::vector<SNode*> nodes; //List of SNodes for the Network
			Network *genesis(int, Network);
	};*/
